...
import { adminColumns as existingColumns, LoanDetailsAdmin } from "../utils/admincolumns";
...
function AdminDashboard() {
    ...
    const handleAccept = async (id: string) => {
        try {
            const response = await axios.patch(`http://localhost:3000/loan/${id}/accept`);
    ...
    const adminColumns = [
        ...existingColumns,
        {
            title: "Actions",
            key: "actions",
            render: (text: any, record: LoanDetailsAdmin) => (
                <div>
                    <Button onClick={() => handleAccept(record.key)}>Accept</Button>
                    <Button onClick={() => handleReject(record.key)} danger>
                        Reject
                    </Button>
                </div>
            ),
