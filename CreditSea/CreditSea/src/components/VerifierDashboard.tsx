...
import { LoanDetailsVerifier, verifierColumns as originalVerifierColumns } from "../utils/verifiercolumns";
...
const verifierColumns = [
  ...originalVerifierColumns,
  {
    title: "Actions",
    key: "actions",
    render: (text: any, record: LoanDetailsVerifier) => (
      <div>
        <button onClick={() => handleVerify(record.key)} className="text-green-500">Verify</button>
        <button onClick={() => handleReject(record.key)} className="text-red-500 ml-2">Reject</button>
      </div>
    ),
  },
];
            ...
            if (data.data !== undefined) {
                ...
                for (let i of data.data) {
                    result.push({
                        ...
                        "Action": i.loanStatus,
                        "key": i.id  // Use the ID from backend instead of generating new UUID
                    });
                }
                setLoanData(result);
            }
